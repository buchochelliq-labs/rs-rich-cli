...
```bash
rich --diff before.png after.png --threshold 5.39 \
...
fixtures. The pictured threshold verdict is computed by the CLI. Both percentages
use the same displayed rounding, so a `5.39` threshold prints as `5.4%` and agrees
with the `OK` verdict. See [threshold behavior](../image-diff.md#as-a-ci-gate).
...
versions in fresh consumers after publishing. Upload and registry verification
reuse one checkout in the protected `crates-io` job. A manual `verify_only` run
can retry registry verification without repeating uploads, while retaining the
annotated-tag, main-ancestry and full-CI gates. Lightweight tags are rejected.
