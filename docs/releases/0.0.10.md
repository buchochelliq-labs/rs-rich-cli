...
**Published 2026-09-23** through crates.io Trusted Publishing. The cohort is core
0.0.6, ext 0.0.8, art 0.0.8 and CLI 0.0.10; see [Publication](#publication). The
previous [0.0.9 cohort](0.0.9-expanded.md) is also published.
...
The patched consumer stood in for registry resolution before publication; the release
runs' exact-version registry consumers have since passed (see [Publication](#publication)).
...
Independent, annotated tags on `main` at `e604cc5` (#200). Each release run published
through Trusted Publishing (environment `crates-io`) and then built an exact-version
registry consumer. No coordinated `v0.0.10` tag exists; the release script rejects it.

| Tag | Release run | crates.io |
|---|---|---|
| `rs-rich-v0.0.6` | [35818053978](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/35818053978) (attempt 2) | [rs-rich 0.0.6](https://crates.io/crates/rs-rich/0.0.6) |
| `rs-rich-ext-v0.0.8` | [35818015980](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/35818015980) (attempt 2) | [rs-rich-ext 0.0.8](https://crates.io/crates/rs-rich-ext/0.0.8) |
| `rs-rich-art-v0.0.8` | [35818038760](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/35818038760) (attempt 2) | [rs-rich-art 0.0.8](https://crates.io/crates/rs-rich-art/0.0.8) |
| `rs-rich-cli-v0.0.10` | [35818067984](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/35818067984) (attempt 2) | [rs-rich-cli 0.0.10](https://crates.io/crates/rs-rich-cli/0.0.10) |

**Tag-order incident.** All four tags were pushed within about 40 seconds, with
`rs-rich-ext-v0.0.8` first. Its first attempt failed the pre-publish dry run because
`rs-rich ^0.0.6` was not yet on crates.io; nothing was uploaded. The other runs queued
behind the `crates-io-release` concurrency group, and GitHub keeps only one pending run
per group, so the queued runs were cancelled. Recovery re-ran the tag runs one at a
time in dependency order (core, then ext and art, then CLI). Manual `workflow_dispatch`
runs started during recovery were superseded and cancelled before publishing. Every
version was uploaded exactly once. Pushing tags in order, waiting for each run, avoids
this; see [Registry authentication](../BRANCHING.md#registry-authentication-trusted-publishing).

The old `CARGO_REGISTRY_TOKEN` repository secret is no longer used and should be deleted.
...
| #198 tracker | Open | Published: close |
