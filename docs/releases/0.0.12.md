...
**Published 2026-09-26.** Every workstream in the
[0.0.12 plan](../plans/0.0.12.md) shipped, and the release test below passed on
the integrated tree. All eight crates are on crates.io, three of them new
(`rs-rich-plugin-api`, `rs-rich-mermaid`, `rs-rich-lumis`), and the first
`rs-rich` Python package is on PyPI. See [Publication](#publication).
...
### Known gap: rich-cli options

Screenshot checks against upstream rich-cli 1.8.1 found that the `rich`
binary (and `rich-rs`, which runs it) still lacks some of rich-cli's rendering
options: `--head`/`--tail`, `-n`/`--line-numbers`, `--guides`, `--lexer`,
`--emoji`, `--soft`, `--no-wrap`, `--max-width`, the `--text-*` alignments,
`--rule-style`/`--rule-char`, `--rst`, `--force-terminal`, and most short
aliases. This is not new in 0.0.12 (0.0.11 lacks them too); they are planned
for 0.0.13 ([#542](https://github.com/buchochelliq-labs/rs-rich-cli/issues/542)) and listed in [PORTING](../PORTING.md).

## Publication

Independent annotated tags on `main` at `69dc7fe` (#543), published one at a
time in dependency order on 2026-09-26. No coordinated `v0.0.12` tag exists; the
release script rejects it.

| Package | How | Run | Registry |
|---|---|---|---|
| `rs-rich` 0.0.8 | `rs-rich-v0.0.8`, Trusted Publishing | [36233976125](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36233976125) | [crates.io](https://crates.io/crates/rs-rich/0.0.8) |
| `rs-rich-plugin-api` 0.0.1 | By hand with a maintainer API token | — | [crates.io](https://crates.io/crates/rs-rich-plugin-api/0.0.1) |
| `rs-rich-macros` 0.0.2 | `rs-rich-macros-v0.0.2`, Trusted Publishing | [36234588352](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36234588352) | [crates.io](https://crates.io/crates/rs-rich-macros/0.0.2) |
| `rs-rich-ext` 0.0.10 | `rs-rich-ext-v0.0.10`, Trusted Publishing | [36234677455](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36234677455) (attempt 2) | [crates.io](https://crates.io/crates/rs-rich-ext/0.0.10) |
| `rs-rich-art` 0.0.10 | `rs-rich-art-v0.0.10`, Trusted Publishing | [36234686496](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36234686496) | [crates.io](https://crates.io/crates/rs-rich-art/0.0.10) |
| `rs-rich-lumis` 0.0.1 | By hand with a maintainer API token | — | [crates.io](https://crates.io/crates/rs-rich-lumis/0.0.1) |
| `rs-rich-mermaid` 0.0.1 | By hand with a maintainer API token | — | [crates.io](https://crates.io/crates/rs-rich-mermaid/0.0.1) |
| `rs-rich-cli` 0.0.12 | `rs-rich-cli-v0.0.12`, Trusted Publishing | [36235801321](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36235801321) | [crates.io](https://crates.io/crates/rs-rich-cli/0.0.12) |
| `rs-rich` 0.0.1 (Python) | `python-v0.0.1`, PyPI Trusted Publishing from the `pypi` environment | [36236294253](https://github.com/buchochelliq-labs/rs-rich-cli/actions/runs/36236294253) | [PyPI](https://pypi.org/project/rs-rich/0.0.1/) |

**Why three crates were uploaded by hand.** crates.io offers Trusted
Publishing only on a crate that already exists, so a new crate's first version
needs a maintainer token. Each went up once its own dependencies were on
crates.io: the plugin API after core 0.0.8 (and before ext, which depends on
it), lumis after the plugin API, and Mermaid after art 0.0.10 (its optional
`mmdc` dependency, which crates.io resolves at upload too). All three came
before the CLI tag. Later versions publish from the workflow once their Trusted
Publishing entries are added. See
...
then built an exact-version registry consumer. Independently, from the
registries themselves (no path patches):

- `cargo install --locked rs-rich-cli --version =0.0.12` reports 0.0.12 and
  draws a Mermaid flowchart;
- a new crate pinned to `=0.0.8` core, `=0.0.10` ext (`data`, `yaml`,
  `macros`) and art, `=0.0.1` plugin API, Mermaid and lumis, and `=0.0.2`
  macros resolves every one from crates.io, registers `MermaidPlugin` and
  `LumisPlugin`, routes Mermaid fences into `Markdown`, and parses YAML;
- `pip install rs-rich==0.0.1` in a fresh Python 3.13 venv installs the
  manylinux wheel (PyPI carries wheels for Linux x86-64 and arm64, macOS arm64
  and x86-64, Windows x86-64, and the sdist); `rich-rs --version` reports the
  0.0.12 CLI, and the whole Python suite passes against it (1,051 passed,
  2 skipped, with `rich==15.0.0` as the reference).
