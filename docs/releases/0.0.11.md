...
- **Config trust.** A working-directory `rich.toml` can no longer set
  `theme_file` or turn off sanitising. Theme files must be regular files of at
  most 1 MiB.
...
| #126 image transforms | Open | The alpha and checkerboard backgrounds were its last planned item (#516): close |
...
- **Maintainer decisions.**
  - `export_html`/`export_svg` in a working-directory `rich.toml` can still
    write files, as since 0.0.10. The release test did not change it, because
    that would break existing projects. Say whether it should get the
    untrusted-layer rule.
