...
| #126 image transforms | Open | Everything shipped (fit, max sizes, anchors, transforms, adjustments, alpha backgrounds) except native sizing: no mode keeps an image at its own pixel size, so an 8×8 image fills the terminal width. Keep open for that, or split it out and close |
