...
| `rs-rich-lumis` (new in 0.0.12, not yet published) | `rich_lumis` | The lumis syntax highlighter (tree-sitter, over 100 languages, 250+ Neovim themes, plus `ansi_dark`/`ansi_light`), for `Syntax`, Markdown or the plugin host. | [Plugins](https://github.com/buchochelliq-labs/rs-rich-cli/blob/main/docs/PLUGINS.md) |
| `rs-rich-mermaid` (new in 0.0.12, not yet published) | `rich_mermaid` | Mermaid diagrams: flowcharts drawn as text in every direction, every diagram type through `mmdc` (optional), and ```` ```mermaid ```` fences in Markdown. | [Plugins](https://github.com/buchochelliq-labs/rs-rich-cli/blob/main/docs/PLUGINS.md) |
