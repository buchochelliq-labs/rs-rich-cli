...
image       Render a still image as ASCII/blocks/Sixel (`--image`)
...
-x, --syntax     Syntax-highlight RESOURCE (language from its extension)
    ...
    --image      Render RESOURCE as a still image (ASCII/blocks/Sixel)
```
...
-w, --width N    Render the output N columns wide (the console keeps its
    ...
    --image-mode M
                 ...
                 (default), sixel (real pixels), blocks, ascii, none
