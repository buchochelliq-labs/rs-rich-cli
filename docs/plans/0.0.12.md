...
- **PyPI publishing** through PyPI's own Trusted Publishing, from a separate
  tag (`richer-v0.0.1`).
...
- **Mermaid scope: open.** The options:
  1. Native text rendering, flowcharts only (workstream 3 as written).
  2. Native text rendering, flowcharts plus sequence diagrams. Sequence
     diagrams lay out as columns and are the next most common type.
  3. An optional external `mmdc` backend (Node and headless Chromium) that
     renders any diagram type to an image, shown through `rich-art`, with the
     native renderer as the fallback.
  4. An optional Kroki backend (an HTTP service) for the same, which sends
     the diagram source over the network.

  For layout, either write a small layered layout or build on `ascii-dag`
  (MIT/Apache, text output, handles cycles) or `rust-sugiyama` (MIT,
  coordinates only). Recommended: option 2, with option 3 as a later,
  off-by-default feature.
- **The Python wrapper (#197) stays in 0.0.12** (decided 2026-09-24) as
  workstream 7, first slice only.
  - Package and import names: default `richer-cli` on PyPI and `import
    richer`, as #197 suggests. We don't claim the `rich` namespace.
...
| `richer-cli` (PyPI) | 0.0.1 (new) | Python bindings, first slice; `crates/rich-py` is not on crates.io |
...
6. CLI;
7. `richer-cli` on PyPI, from its own tag after core is on crates.io.

The CLI depends on the lumis and Mermaid crates only through off-by-default
features. It still needs them on crates.io before its own upload.
