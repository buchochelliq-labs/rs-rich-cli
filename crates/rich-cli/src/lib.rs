...
//!
//! The command line lives in this library target so the Python wheel can run
//! it in-process ([`run`] and [`run_embedded`] are its only public items);
//! `main.rs` is the `rich` binary and only calls [`run`].
//!
...
/// Run the `rich` command line with `args` (the arguments after the program
/// name) and return its exit status. The `rich` binary's `main` is only this.
///
/// This library target exists for the Python wheel (`crates/rich-py`), which
/// runs the CLI in-process through [`run_embedded`]; it is not a supported
/// Rust API, and everything else in it is private. Output goes to the
/// process's real stdout and stderr and input comes from its stdin; some modes
...
/// A command that runs this CLI again: a `--batch` worker, or the demo's
/// child `--watch`. In the `rich` binary that is the current executable; under
/// [`run_embedded`] it is the host's command.
fn self_command() -> std::io::Result<std::process::Command> {
    let program = SELF_PROGRAM
        .lock()
        .map(|program| program.clone())
        .unwrap_or_default();
    Ok(match program.split_first() {
