...
//!
//! Each subcommand completes its own options plus the [`ArgSpec::global`]
//! options of the commands above it.
//!
//! Commands are tracked by internal node ids (`n0`, `n1`, …) so names never
//! need escaping into patterns or function names. The command name itself
//! loses whitespace and control characters wherever a script names it (a
//! newline in `#compdef` would start a new command), and words reach fish
//! only as `printf` output, which fish does not expand again.
...
struct Node<'a> {
    ...
    /// The visible arguments completed here: the command's own, then the
    /// global options of the commands above it that it does not redefine.
    args: Vec<&'a ArgSpec>,
}
...
fn nodes(root: &CommandSpec) -> Vec<Node<'_>> {
    ...
    while i < out.len() {
        ...
        for child in spec.visible_subcommands() {
            ...
            // clap copies global arguments into subcommands itself; an
            // argument the child already has (by id) is not added again.
...
/// The command name as scripts write it: whitespace and control characters
/// removed, so it stays one word on one line (`#compdef`, comments).
...
/// A word in an `_arguments` action list, which zsh evaluates: every ASCII
/// character but letters, digits and `-_./,+@` is backslash-escaped, so
/// `;`, `|`, `&`, `<`, `>`, a leading `=` or `~` and quotes stay literal.
...
fn fish(root: &CommandSpec, nodes: &[Node<'_>]) -> String {
    ...
    // Value and subcommand lists live in functions: `printf` lines of
    // `word<TAB>help`. fish expands `complete -a` arguments again, so a word
    // written there as `(cmd)` or `$(cmd)` would run; printed output is not.
