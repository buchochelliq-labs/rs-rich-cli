...
evidence = ROOT / '.github/evidence/0.0.8/demo-tour'
...
media.encode(frames, 'v8-demo-tour')
poster_time = next(event[0] for event in events if 'Half-block' in event[2]) + .25
frames[min(len(frames) - 1, round(poster_time * media.FPS))].save(media.OUT / 'v8-demo-tour.png')
