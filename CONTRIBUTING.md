...
- The dependency arrow is one-way: `rich-cli → rich-ext → rich` and
  ...
  `rich-cli → rich-mermaid → rich-plugin-api`, and `rich-lumis → rich-plugin-api`.
  Core depends on none of them.
- Each crate (`rs-rich`, `rs-rich-plugin-api`, `rs-rich-macros`, `rs-rich-ext`,
  `rs-rich-art`, `rs-rich-mermaid`, `rs-rich-lumis`, `rs-rich-cli`) has its **own independent SemVer**, bumped
