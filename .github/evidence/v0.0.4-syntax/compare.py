...
parser=argparse.ArgumentParser(description='Compare syntax behavior and exports against a pre-change binary')
...
fixtures={
 'state.rs':'let x = 1;\n/*\nlet x = 1;\n*/\nlet x = 1;\nlet x = 1;\n',
 'raw.rs':'let s = r###"\nvalue\n"##;\nvalue\n"###;\nvalue\nvalue',
 'unicode.py':'#!/usr/bin/env python3\nx = "漢字🙂é"\ns = """\nx = "漢字🙂é"\n"""\nx = "漢字🙂é"\nx = "漢字🙂é"\n',
 'heredoc.rb':'s = <<ONE\nvalue\nTWO\nvalue\nONE\nvalue\nvalue\n',
 'embedded.html':'value\n<script>\nvalue\nlet x = 1;\nlet x = 1;\n</script>\nvalue\nvalue\n',
 'empty.rs':'',
}

variants=[[],['--panel','rounded'],['--padding','1'],['--center'],['--right'],['--pager'],['--panel','rounded','--title','[bold]Code[/]'],['--panel','rounded','--style','italic']]
results=[]
  ...
  for width in [4,20,80]:
   for flags in variants:
    ...
    for binary in [baseline,current]:
     ...
     cmd=[str(binary),'--syntax',str(path),'--width',str(width),*flags,'--export-html',str(html),'--export-svg',str(svg)]
